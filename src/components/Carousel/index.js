...
import {
  addClass,
  removeClass,
} from "./../../helpers/format/classNameModifier";

export default function Carousel({ children, refContainer }) {
  const refDragHandler = React.useRef(null);
  ...
  const posInitial = React.useRef();
  const posX1 = React.useRef();
  const posX2 = React.useRef();
  const posFinal = React.useRef();
  const isAllowShift = React.useRef(true);
  const cards = React.useRef();
  const cardCount = cards.current?.length || 0;
  const cardSize = cards.current?.[0].offsetWidth || 0;

  const fnCheckIndex = React.useCallback(
    (e) => {
  ...
  const fnShiftItem = React.useCallback(
    (direction) => {
